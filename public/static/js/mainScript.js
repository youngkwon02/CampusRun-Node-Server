...
const acceptInvitation = async (invId, url) => {
  ...
  let roomUrl = url;
  await ajaxRequest("GET", "http://172.30.1.33:8000/game/api/room-enter", {
    roomUrl,
    kakaoId,
  });
  let res = await ajaxRequest(
    "GET",
    "http://172.30.1.33:8000/game/api/invitation-read",
    { invId: invId }
  );
  location.href = url;
};
...
const rejectInvitation = async (invId) => {
  let res = await ajaxRequest(
    "GET",
    "http://172.30.1.33:8000/game/api/invitation-reject",
    { invId: invId }
  );
  if (res.status !== 200)
...
const invitationManager = async (kakaoId) => {
  ...
  let res = await ajaxRequest(
    "GET",
    "http://172.30.1.33:8000/game/api/invitation-by-id",
    { kakaoId: kakaoId }
  );
...
const nicknameCreate = async (kakaoId) => {
  ...
  let createNickName = await ajaxRequest(
    "GET",
    "http://172.30.1.33:8000/api/create-nickname",
...
const nicknameCheck = async (kakaoId) => {
  ...
  let verifyNickName = await ajaxRequest(
    "GET",
    "http://172.30.1.33:8000/api/check-nickname",
    { kakaoId }
  );
