const createPublicRoomAction = async () => {
  ...
  let createRoomRes = await ajaxRequest(
    "GET",
    "http://172.30.1.33:8000/game/api/create-room-public",
...
const updatePublicList = async () => {
  ...
  let userResponse = await ajaxRequest(
    "GET",
    "http://172.30.1.33:8000/api/user-by-kakaoid",
    { kakaoId: KAKAOID }
  );
  if (userResponse) {
  ...
  let publicRoomRes = await ajaxRequest(
    "GET",
    "http://172.30.1.33:8000/game/api/public-room-list",
...
const enterWaitRoom = async (waitingURL) => {
  ...
  let enterWaitingRes = await ajaxRequest(
    "GET",
    "http://172.30.1.33:8000/game/api/enter-wait-room",
    {
      kakaoId: KAKAOID,
      waitingURL,
    }
